...
2. **工具验证** — 所有 payload 必须用 curl 或 python3 工具实际发送验证，不猜测结果
...
2. **如源码含 `highlight_file` → 用 python3 + strip_tags 提取纯源码**（fetch 输出可能误读）
...
4. **拿到 base64 后用 python3 编码解码 工具解码** — 不要自己脑补解码结果
...
- 用 python3 工具构造请求，而非 fetch 工具（fetch 可能不支持多参数）
